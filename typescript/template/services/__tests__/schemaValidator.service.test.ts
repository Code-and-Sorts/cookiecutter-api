...
describe('SchemaValidator', () => {
    ...
    const mockValid{{project_class_name}} = {
        name: 'mock{{project_class_name}}',
    };
    const mockInvalid{{project_class_name}}1 = {
        name: 'mock{{project_class_name}}',
        invalidProp: 'mockInvalidProp',
    };
    const mockInvalid{{project_class_name}}2 = {
        name: 'mock{{project_class_name}}',
        invalidProp1: {
            invalidProp: 'mockInvalidProp',
        },
    ...
    it('should successfully throw for invalid schema', () => {
        try {
            mockSchemaValidator.validate(mockInvalid{{project_class_name}}1, BaseSchema);
        } catch (error) {
    ...
    it('should successfully throw for multiple unrecognized keys', () => {
        try {
            mockSchemaValidator.validate(mockInvalid{{project_class_name}}2, BaseSchema);
        } catch (error) {
            expect(error).toBeInstanceOf(ValidationError);
            expect(error.statusCode).toEqual(422);
            expect(error.message).toEqual('Failed schema validation');
