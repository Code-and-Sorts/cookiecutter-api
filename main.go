...
func handler(ctx context.Context, request events.APIGatewayProxyRequest) (events.APIGatewayProxyResponse, error) {
	...
	switch endpoint {
	case "kitties":
		return dispatchKittenClaws(ctx, request, needsID)
	default:
		return utils.GenerateErrorResponse("Not Found", 404), nil
	}
}

func dispatchKittenClaws(ctx context.Context, request events.APIGatewayProxyRequest, needsID bool) (events.APIGatewayProxyResponse, error) {
