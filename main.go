...
func handler(ctx context.Context, request events.APIGatewayProxyRequest) (events.APIGatewayProxyResponse, error) {
	...
	switch endpoint {
	case "cats":
		return dispatchCat(ctx, request, needsID)
	case "dogs":
		return dispatchDog(ctx, request, needsID)
	default:
		return utils.GenerateErrorResponse("Not Found", 404), nil
	}
}

func dispatchCat(ctx context.Context, request events.APIGatewayProxyRequest, needsID bool) (events.APIGatewayProxyResponse, error) {
...
func dispatchDog(ctx context.Context, request events.APIGatewayProxyRequest, needsID bool) (events.APIGatewayProxyResponse, error) {
	switch request.HTTPMethod {
	case "GET":
		if needsID {
