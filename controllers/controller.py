from typing import List, Optional
import azure.functions as func


from services import (
    CatService,
    DogService,
)
from repositories.repository import DEFAULT_LIST_LIMIT
from models import (
    CatResponse,
    Cat,
    CatIdValidation,
    DogResponse,
    Dog,
    DogIdValidation,
)

MAX_LIST_LIMIT = 1000


def _coerce_limit(raw: Optional[str]) -> int:
    try:
        limit = int(raw)
    except (TypeError, ValueError):
        return DEFAULT_LIST_LIMIT
    if limit < 1:
        return DEFAULT_LIST_LIMIT
    return min(limit, MAX_LIST_LIMIT)


class CatController:
    def __init__(self, service: CatService):
        self.service = service

    async def get_by_id(self, req: func.HttpRequest) -> CatResponse:
        item_id: str = req.route_params.get('item_id')
        CatIdValidation(id=item_id)
        return await self.service.get_by_id(item_id)

    async def get_list(self, req: func.HttpRequest) -> List[CatResponse]:
        limit = _coerce_limit(req.params.get('limit'))
        return await self.service.get_list(limit)

    async def create(self, req: func.HttpRequest) -> CatResponse:
        item_json: dict = req.get_json()
        item = Cat(**item_json)
        return await self.service.create(item)

    async def update(self, req: func.HttpRequest) -> CatResponse:
        item_id: str = req.route_params.get('item_id')
        item_data: dict = req.get_json()
        item = Cat(**{**item_data, "id": item_id})
        return await self.service.update(item)

    async def soft_delete(self, req: func.HttpRequest) -> None:
        item_id: str = req.route_params.get('item_id')
        await self.service.soft_delete(item_id)


class DogController:
    def __init__(self, service: DogService):
        self.service = service

    async def get_by_id(self, req: func.HttpRequest) -> DogResponse:
        item_id: str = req.route_params.get('item_id')
        DogIdValidation(id=item_id)
        return await self.service.get_by_id(item_id)

    async def get_list(self, req: func.HttpRequest) -> List[DogResponse]:
        limit = _coerce_limit(req.params.get('limit'))
        return await self.service.get_list(limit)

    async def create(self, req: func.HttpRequest) -> DogResponse:
        item_json: dict = req.get_json()
        item = Dog(**item_json)
        return await self.service.create(item)

    async def replace(self, req: func.HttpRequest) -> DogResponse:
        item_id: str = req.route_params.get('item_id')
        item_data: dict = req.get_json()
        item = Dog(**{**item_data, "id": item_id})
        return await self.service.replace(item)

    async def soft_delete(self, req: func.HttpRequest) -> None:
        item_id: str = req.route_params.get('item_id')
        await self.service.soft_delete(item_id)

