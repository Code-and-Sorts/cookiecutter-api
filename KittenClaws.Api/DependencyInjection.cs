...
public static class DependencyInjection
{
    ...
    public static IServiceCollection AddPersistence(this IServiceCollection services, IConfiguration configuration)
    {
        ...
        services.AddSingleton<ICatController>(provider =>
        {
            ...
            string containerName = configuration.GetValue<string>("CosmosDbContainerName_Animals") ?? "animals";
            var repository = new CatRepository(cosmosClient, databaseName, containerName);
        ...
        services.AddSingleton<IDogController>(provider =>
        {
            var cosmosClient = provider.GetRequiredService<CosmosClient>();
            string containerName = configuration.GetValue<string>("CosmosDbContainerName_Animals") ?? "animals";
            var repository = new DogRepository(cosmosClient, databaseName, containerName);
