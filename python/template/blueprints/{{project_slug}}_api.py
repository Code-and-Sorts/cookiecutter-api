...
async def _run(operation):
    ...
    async with CosmosClient(settings.cosmos_db_uri, settings.cosmos_db_key) as client:
        ...
        repository = {{ project_class_name }}Repository(container_client)
        service = {{ project_class_name }}Service(repository)
        controller = {{ project_class_name }}Controller(service)
...
async def _run(operation):
    ...
    try:
        ...
        repository = {{ project_class_name }}Repository(collection)
        service = {{ project_class_name }}Service(repository)
        controller = {{ project_class_name }}Controller(service)
        return await operation(controller)
    finally:
...
repository = {{ project_class_name }}Repository(
    session,
...
{% if cloud_service == 'Azure Function App' -%}
@bp.route(route="{{ project_endpoint }}/{item_id}", methods=[func.HttpMethod.GET])
...
{%- endif %}
    logging.info("Get {{ project_endpoint }} by ID processed a request.")
...
{% if cloud_service == 'Azure Function App' -%}
@bp.route(route="{{ project_endpoint }}", methods=[func.HttpMethod.GET])
...
{%- endif %}
    logging.info("Get {{ project_endpoint }} list processed a request.")
...
{% if cloud_service == 'Azure Function App' -%}
@bp.route(route="{{ project_endpoint }}", methods=[func.HttpMethod.POST])
...
{% if cloud_service == 'Azure Function App' -%}
@bp.route(route="{{ project_endpoint }}/{item_id}", methods=[func.HttpMethod.PATCH])
...
{% if cloud_service == 'Azure Function App' -%}
@bp.route(route="{{ project_endpoint }}/{item_id}", methods=[func.HttpMethod.DELETE])
...
{%- if cloud_service == 'Azure Function App' %}
        ...
        return func.HttpResponse(
            body="{{ project_class_name }} deleted.",
            status_code=200
        )
{%- endif %}
{%- if cloud_service == 'GCP Cloud Function' %}
        asyncio.run(_run(lambda c: c.soft_delete(request)))
        return ("{{ project_class_name }} deleted.", 200)
{%- endif %}
{%- if cloud_service == 'AWS Lambda' %}
        ...
        return {
            "statusCode": 200,
            "headers": {"Content-Type": "application/json"},
            "body": '{"message": "{{ project_class_name }} deleted."}'
        }
{%- endif %}
