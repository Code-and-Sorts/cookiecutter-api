...
{%- elif cookiecutter.cloud_service == 'Google Cloud Function' %}
    ...
    def create(self, item_json: dict) -> {{ cookiecutter.project_class_name }}Response:
        item = {{ cookiecutter.project_class_name }}(**item_json)
        return self.service.create(item)

    def update(self, item_id: str, item_data: dict) -> {{ cookiecutter.project_class_name }}Response:
        item = {{ cookiecutter.project_class_name }}(**item_data)
        item.id = item_id
        return self.service.update(item)
