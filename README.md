...
|---|---|---|---|
| `kitties` | `DYNAMODB_TABLE_NAME_KITTIES` | `KittiesTable` | KittenClaws |
...
> **Upgrading from a single-resource project:** the storage setting name now includes the container id. `DYNAMODB_TABLE_NAME` became `DYNAMODB_TABLE_NAME_<CONTAINER>` (see the table above), so rename it in every deployed environment.
