...
Settings are read from environment variables (case-insensitive). The table names are wired to the tables created in `template.yaml`.

| Variable | Description | Default |
| --- | --- | --- |
| `AWS_REGION` | AWS region | `us-east-1` |
| `DYNAMODB_TABLE_NAME_KITTY_CATS` | DynamoDB table for `kitty_cats` | `kitty_cats` |

> **Note:** earlier versions of this template used a single `DYNAMODB_TABLE_NAME` variable. It has been replaced by one variable per container, listed above.
...
4. Run the API Locally
    ...
    > `DYNAMODB_TABLE_NAME_KITTY_CATS`.
