...
|---|---|---|---|
| `kitties` | `DYNAMODB_TABLE_NAME_KITTIES` | `kitties` | KittenClaws |
...
> `DYNAMODB_TABLE_NAME` is now `DYNAMODB_TABLE_NAME_<CONTAINER>`, one per container (for example `DYNAMODB_TABLE_NAME_KITTIES`).
...
| `DYNAMODB_TABLE_NAME_KITTIES` | no | DynamoDB table for `kitties` (default `kitties`) |
