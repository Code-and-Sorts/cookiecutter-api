...
- **Cat** (container: `animals`)
  - `GET /api/cats` — list
  - `GET /api/cats/{item_id}` — get by ID
  - `POST /api/cats` — create
  - `PATCH /api/cats/{item_id}` — partial update
  - `DELETE /api/cats/{item_id}` — soft delete
- **Dog** (container: `animals`)
  - `GET /api/dogs` — list
  - `GET /api/dogs/{item_id}` — get by ID
  - `POST /api/dogs` — create
  - `PUT /api/dogs/{item_id}` — full replace
  - `DELETE /api/dogs/{item_id}` — soft delete
...
Settings are read from environment variables (case-insensitive). For local development set them in `local.settings.json`.
...
| `Container_Name_animals` | Cosmos DB container for `animals` | `animals` |

> **Note:** earlier versions of this template used a single `COSMOS_DB_CONTAINER_NAME` variable. It has been replaced by one variable per container, listed above.
