...
- **KittenClaws** (container: `kitty_cats`)
  ...
  - `GET /api/kitties/{item_id}` — get by ID
  - `POST /api/kitties` — create
  - `PATCH /api/kitties/{item_id}` — partial update
  - `DELETE /api/kitties/{item_id}` — soft delete
...
| `Container_Name_kitty_cats` | Cosmos DB container for `kitty_cats` | `kitty_cats` |
