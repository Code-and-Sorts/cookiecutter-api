...
describe('KittenClawsRepository', () => {
    beforeEach(() => {
        jest.resetAllMocks();
    });

    const mockCosmosClient = new MockCosmosClient() as unknown as CosmosClient;
    const mockRepository = new KittenClawsRepository(mockCosmosClient.database('mock-db').container('mock-container'));
    const mockId = '28535ae3-2f1b-4e81-ba13-0f46a0c74ea0';
    const mockItem = { name: 'mockKittenClaws' };
    ...
    const mockItemUpdate = { name: 'mockKittenClawsUpdate' };
