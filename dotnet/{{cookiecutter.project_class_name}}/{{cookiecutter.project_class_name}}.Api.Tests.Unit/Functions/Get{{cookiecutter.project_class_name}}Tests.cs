...
public class Get{{cookiecutter.project_class_name}}Test
{
    ...
    public async Task Get_ReturnsGetResult_When{{cookiecutter.project_class_name}}IsGot()
    {
        ...
        var get{{cookiecutter.project_class_name}}Dto = new {{cookiecutter.project_class_name}}Dto { Name = "mock{{cookiecutter.project_class_name}}" };
        var httpRequestData = Mocks.CreateHttpRequestData(get{{cookiecutter.project_class_name}}Dto, "GET");

        _mock{{cookiecutter.project_class_name}}Controller.GetAsync({{cookiecutter.project_lower_camel_name}}Id, Arg.Any<CancellationToken>()).Returns(Task.FromResult(get{{cookiecutter.project_class_name}}Dto));

        // Act
        var result = await _get{{cookiecutter.project_class_name}}Function.Get(httpRequestData, {{cookiecutter.project_lower_camel_name}}Id);

        var getResult = Assert.IsType<OkObjectResult>(result);

        // Assert
        Assert.Equal(200, getResult.StatusCode);
        Assert.Equal(get{{cookiecutter.project_class_name}}Dto, getResult.Value);
    }
    ...
    public async Task Get_ReturnsErrorResult_WhenExceptionIsThrown()
    {
        ...
        _mock{{cookiecutter.project_class_name}}Controller.GetAsync({{cookiecutter.project_lower_camel_name}}Id, Arg.Any<CancellationToken>()).Throws(new Exception("Mock exception"));
