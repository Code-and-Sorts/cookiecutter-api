...
public class Create{{project_class_name}}(I{{project_class_name}}Controller {{project_lower_camel_name}}Controller, ILogger<Create{{project_class_name}}> logger)
{
    ...
    public async Task<APIGatewayProxyResponse> Post(APIGatewayProxyRequest request)
    {
        ...
        try
        {
            ...
            var new{{project_class_name}}Dto = await _{{project_lower_camel_name}}Controller.CreateAsync(bodyStream);

            return ResponseHelper.Created(new{{project_class_name}}Dto);
        }
        catch (Exception ex)
        {
