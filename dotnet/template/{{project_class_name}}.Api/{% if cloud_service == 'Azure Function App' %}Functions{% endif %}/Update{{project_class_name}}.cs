...
public class Update{{project_class_name}}(I{{project_class_name}}Controller {{project_lower_camel_name}}Controller, ILogger<Update{{project_class_name}}> logger)
{
    ...
    [Function("Update{{project_class_name}}")]
    public async Task<IActionResult> Patch(
        [HttpTrigger(AuthorizationLevel.Function, "patch", Route = "{{project_endpoint}}/{id}")] HttpRequestData update{{project_class_name}}Request, string id, CancellationToken ct = default)
    {
        _logger.LogInformation($"{nameof(Update{{project_class_name}})} processed a request.");

        try
        {
            var updated{{project_class_name}}Dto = await _{{project_lower_camel_name}}Controller.UpdateAsync(id, update{{project_class_name}}Request.Body, ct);

            return new OkObjectResult(updated{{project_class_name}}Dto);
        }
        catch (Exception ex)
        {
