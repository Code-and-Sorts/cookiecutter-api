...
public class Get{{cookiecutter.project_name}}List
{
    private readonly I{{cookiecutter.project_name}}Controller _{{cookiecutter.project_lower_camel_name}}Controller;
    private readonly ILogger<Get{{cookiecutter.project_name}}List> _logger;

    public Get{{cookiecutter.project_name}}List(I{{cookiecutter.project_name}}Controller {{cookiecutter.project_lower_camel_name}}Controller, ILogger<Get{{cookiecutter.project_name}}List> logger)
    {
        _{{cookiecutter.project_lower_camel_name}}Controller = {{cookiecutter.project_lower_camel_name}}Controller;
        _logger = logger;
    }

    [FunctionName("Get{{cookiecutter.project_name}}List")]
    public async Task<IActionResult> Get(
        [HttpTrigger(AuthorizationLevel.Function, "get", Route = "{{cookiecutter.project_endpoint}}")] CancellationToken ct = default)
    {
        _logger.LogInformation($"{nameof(Get{{cookiecutter.project_name}}List)} processed a request.");
        ...
        {
            _logger.LogError(ex, "Exception in {ClassName} -> {MethodName} method.", nameof(Get{{cookiecutter.project_name}}List), nameof(Get));
